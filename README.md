...
![Lambda_1](https://latex.codecogs.com/svg.image?$\Lambda_{1}=\begin{cases}0&\text{if}l_{i}=-\infty,\,u_{i}=&plus;\infty,\\[5pt]\mathbb{R}^{-}&\text{if}l_{i}=-\infty,\,u_{i}\in\mathbb{R},\\[5pt]\mathbb{R}^{&plus;}&\text{if}l_{i}\in\mathbb{R},\,u_{i}=&plus;\infty,\\[5pt]\mathbb{R}&\text{otherwise}\end{cases}$)
