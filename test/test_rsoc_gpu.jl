...
m_rsoc = m - m_zero - m_nonnegative # number of second-order cone constraints
c = ones(n) * 10
...
@constraint(model, (A * x - b)[m_zero+m_nonnegative+1:end] in MOI.RotatedSecondOrderCone(m_rsoc))
optimize!(model)