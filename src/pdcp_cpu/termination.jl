...
function optimality_criteria_met(;
    ...
    if max(info.l_2_rel_primal_res, info.l_2_rel_dual_res, info.rel_gap) < rel_tol
        info.status = :optimal;
    end
    if max(info.l_2_abs_primal_res, info.l_2_abs_dual_res, info.abs_gap) < abs_tol
        info.status = :optimal;
    end
...
function primal_infeasibility_criteria_met(;
    ...
    res = max(infeasibility_info.max_dual_ray_infeasibility, 0.01) / infeasibility_info.dual_ray_objective
    if  res <= eps_primal_infeasible_low_acc
...
function dual_infeasibility_criteria_met(;
    ...
    res = max(infeasibility_info.max_primal_ray_infeasibility, 0.01) / (-infeasibility_info.primal_ray_objective)
    if  res <= eps_dual_infeasible_low_acc
...
function check_termination_criteria(;
    ...
    for i_info in info.infeaInfo
        ...
        if i_info.status == :primal_infeasible_low_acc && info.iter > 2000
            ...
            info.exit_code = 2
            info.pObj = i_info.primal_objective
            info.dObj = i_info.dual_objective
