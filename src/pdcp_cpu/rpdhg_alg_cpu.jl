...
function printInfo(;info::PDHGCLPInfo)
    println(@sprintf("iter: %d primal_res: %.2e dual_res: %.2e primal_obj: %.2e dual_obj: %.2e pd_gap: %.2e time: %.2f restart_times: %d",
...
println(@sprintf("exit_status:          %s", info.status))
println(@sprintf("primal_res:           %.4e", info.primal_res))
println(@sprintf("dual_res:             %.4e", info.dual_res))
println(@sprintf("pd_gap:               %.4e", info.gap))
println(@sprintf("primal_obj:           %.4e", info.primal_obj))
println(@sprintf("dual_obj:             %.4e", info.dual_obj))
